...
#include <vector>
#include <sys/poll.h>
#include <boost/format.hpp>		// Additional library.  From web.
#include <map>
...
std::map<std::string, std::string>		dfs, Color;
std::string title;
...
void Initialize_Defaults(const char *arg)
{
  ...
  // ---------------------------------------------------------------------------
  size_t y;
  if ((y = title.find("???")) != std::string::npos) {
	title.erase(y, 3);
	std::string cmd = arg;
	title.insert(y, cmd);
  }
...
void Perror(const std::string & buf)
{
  Perror(buf.c_str());
}
...
//  code esc¯seq.
// -------------------------------------------------------------------------
const std::string & ConvertColor(std::string & str, ushort line)
{
  std::string word, new_str;
  while (str.length()) {
	// ---------------------------------------------------------------------
...
void ReadConfigFile(bool dependancy)
{
  ...
  std::string path = io::str(format("%s/.%s") % ptr % EXE_FILE);
...
// Parses each token.
// ---------------------------------------------------------------------------
void Output_Token(std::string & strm, size_t idx, std::vector < std::string > &v, ushort cond)
{
  std::string str;
  while (idx < v.size()) {
	if (idx > 4)
...
// -------------------------------------------------------------------------
void ParseString(const std::string & s, const bool dependancy)
{
#ifdef DEBUG
  Out << "'" << s << "'\n";
#endif
  ...
  std::string var0 = (v.size()? v[0] : ""), var1 = (gt1 ? v[1] : ""), var2 = (gt2 ? v[2] : ""), var3 = (gt3 ? v[3] : ""), buf, buf1, sbuf;
  ...
  switch (caseNO) {
	...
	case 1:					// Filename:LineNumber:Message
	  ...
	  // Warning.
	  // ---------------------------------------------------------------------
	  if (var2.find("arning") != std::string::npos) {	// [Ww]arning
	  ...
	  } else if (var2.find("error") != std::string::npos) {
#ifdef DEBUG
		Out << "Reached error\n";
#endif
		...
		if (var3.find("candidate") != std::string::npos) {
#ifdef DEBUG
...
// Reinit this bad boy.
// ---------------------------------------------------------------------------
void OpenFD(char **argv, std::vector < std::string > &v, const bool dependancy)
{
  int p[2];
  if (pipe(p) < 0)
	Perror("pipe()");
	  ...
	  if (client[0].events & POLLRDNORM) {	// New client connection.
		...
		while (!str.empty()) {
		  ...
		  if (x == std::string::npos) {
			...
			std::string strm = str.substr(0, x);
			str.erase(0, x+1);
            last += strm;
...
int main(int argc, char **argv)
{
  ...
  if ((y = obj.rfind('/')) != std::string::npos)
	obj.erase(0, y + 1);
  for (x = 0; x < 4; x++) {
	strm << ' ' << DFS[x] << " --> " << tmp << "\n";
